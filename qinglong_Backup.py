...
def start():
    ...
    if make_targz(files_name, retval):
        ...
        logger.info('运行目录：{}'.format(retval))
        remote_path = f'{retval}/{QLBK_BACKUPS_PATH}'
        logger.info('开始寻找云端目录：{}'.format(remote_path))
        # 云盘目录
        remote_folder = ali.get_folder_by_path(path=remote_path)
        # assert isinstance(remote_folder, ali.BaseFile)
        logger.info('获取到云端目录：{}'.format(remote_folder))
        file_path = f'{retval}/{files_name}'
        logger.info('备份文件路径：{}'.format(file_path))
        # 上传至网盘
        ali.upload_file(file_path, parent_file_id=remote_folder.file_id)
        message_up_time = time.strftime(
            "%Y年%m月%d日 %H时%M分%S秒", time.localtime())
        text = f'已备份至阿里网盘:\n{run_path}{QLBK_BACKUPS_PATH}/qinglong_{now_time}.tar.gz\n' \
               f'\n备份完成时间:\n{message_up_time}\n' \
               f'\n项目: https://github.com/karhow/ql-backup'
        try:
            send('【qinglong自动备份】', text)
        except:
            logger.info("通知发送失败")
        #
        # if delete_other_backup(files_name, retval):
        #     logger.info('清除旧备份成功！')
        # else:
        #     logger.info('清除旧备份失败！')
...
def delete_other_backup(retain_file, retval):
    """
    保留最新备份，删除其他备份
    """
    dirs_list = os.listdir(retval)
    for f in dirs_list:
        if f != retain_file:
            # 文件绝对路径
            file_path = os.path.join(retval, f)
            if os.path.isdir(file_path):
                logger.info("remove dir: {}".format(file_path))
...
if __name__ == '__main__':
    ...
    logger.info('---------' + str(now_time) + ' 备份程序开始执行------------')
